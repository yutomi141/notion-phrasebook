...
import { useStudyCards, useSubmitReview, generateSessionId, makeReviewPayload } from '@/hooks/useStudyCards';
...
export default function PhrasePage() {
  ...
  const { mutate: submitReview, isPending, isError: submitError } = useSubmitReview();
  ...
  if (isLoading) {
    return (
      <div style={{ padding: '32px 24px', color: 'var(--text-secondary)' }}>
        読み込み中...
      </div>
    );
  }
  ...
  if (!cards || cards.length === 0) {
    return (
      <div style={{ padding: '32px 24px' }}>
        ...
        <Link href="/" style={{ color: 'var(--blue)' }}>ホームへ戻る</Link>
      </div>
    );
  }
